...
if __name__ == '__main__':
    ...
    Save LSTM model
    """
    torch.save(rnn_model.rnn.state_dict(), f"{DIR}/rnn_epoch{NUM_EPOCHS}_lr{LEARNING_RATE}_nlayer{NUM_LAYERS}.pth")
    rnn_df = pd.DataFrame(rnn_gen.history)
    rnn_df.to_csv(f'{DIR}/rnn_epoch{NUM_EPOCHS}_lr{LEARNING_RATE}_nlayer{NUM_LAYERS}.csv', index=False) 
